...
_Bool OneNet_DevLink(void)
{
	...
	if(MQTT_PacketConnect(PROID, AUTH_INFO, DEVID, 256, 0, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttPacket) == 0)
	{
		...
		dataPtr = (unsigned char *)ESP8266_GetIPD(250);									//等待平台响应
		if(dataPtr != NULL)
		{
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *json , *json_value1;//定义cJSON
	...
	switch(type)
	{
		case MQTT_PKT_CMD:															//命令下发
			...
			if(result == 0)
			{
				...
				MQTT_DeleteBuffer(&mqttPacket);	//删MQTT包
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				{
					//解析开关值
					//json_value1 = cJSON_GetObjectItem(json,"LED1");
					
					json_value1 = cJSON_GetObjectItem(json,"target");
					UsartPrintf(USART_DEBUG,"json_value1: [%s]\n",json_value1->string);
					UsartPrintf(USART_DEBUG,"json_value1: [%s]\n",json_value1->valuestring);
					if(strstr(json_value1->valuestring,"LED1")!= NULL)
					{
						json_value1 = cJSON_GetObjectItem(json,"value");
						if(json_value1->valueint)
						{
							LED1_ON();//打开LED1
					...
					else if(strstr(json_value1->valuestring,"BUZZ")!= NULL)
					{
						json_value1 = cJSON_GetObjectItem(json,"value");
						if(json_value1->valueint)
						{
					...
					else if(strstr(json_value1->valuestring,"Servo")!= NULL)
					{
						json_value1 = cJSON_GetObjectItem(json,"value");
						if(json_value1->valueint)Servo_open();//打开舵机
						else Servo_close();//关闭舵机
					}
					else if(strstr(json_value1->valuestring,"Fan")!= NULL)
					{
						json_value1 = cJSON_GetObjectItem(json,"value");
						if(json_value1->valueint)
						{
					...
					else if(strstr(json_value1->valuestring,"WaterPump")!= NULL)
					{
						json_value1 = cJSON_GetObjectItem(json,"value");
						if(json_value1->valueint)
						{
				...
				/**	
					if(json_value1->valueint)//json_value大于0且为整数型
					{
						LED1_ON();//打开LED1
					}
					else
					...
					//解析开关值
					json_value2 = cJSON_GetObjectItem(json,"BUZZ_sw");
					if(json_value2->valueint)//json_value大于0且为整数型
					{
						Buzzer_ON();//打开蜂鸣器
					}
					else
